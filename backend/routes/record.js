...
router.patch('/messages/:id', async (req, res) => {
  try {
    ...
    const messagesCollection = db.collection('messages');
    const result = await messagesCollection.updateOne({ _id: messageId }, updates);
...
router.patch('/messages/:id/reply', async (req, res) => {
  try {
    ...
    const newReply = {
      ...
      timestamp: req.body.timestamp || new Date().toISOString()  // Optionally add timestamp
    };
...
router.get('/appointments/single/:id', async (req, res) => {
  try {
    ...
    const appointmentsCollection = db.collection('appointments');
    const appointment = await appointmentsCollection.findOne({ _id: appointmentId });
...
router.patch('/appointments/:id', async (req, res) => {
  try {
    ...
    const appointmentsCollection = db.collection('appointments');
    const result = await appointmentsCollection.updateOne({ _id: appointmentId }, updates);
...
router.delete('/appointments/:id', async (req, res) => {
  try {
    const appointmentId = new ObjectId(req.params.id);
    const appointmentsCollection = db.collection('appointments');
    const result = await appointmentsCollection.deleteOne({ _id: appointmentId });
